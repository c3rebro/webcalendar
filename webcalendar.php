...
if (isset($_GET["edit"])) {
  ...
  $select = $db->prepare("SELECT `id`, `event`, `description`, `color`, `private`,`holiday`, `notification`, `date_today`, `start_date`, `end_date`, `time_today`, `start_time`, `end_time`
                          FROM `calendar`
                          WHERE `id` = :id");
  ...
  // Date and time
  sscanf($event["date_today"], "%4s-%2s-%2s %5s", $dbYearToday, $dbMonthToday, $dbDayToday, $dbTimeToday);
  $date_today = shortDate($dbDayToday, $dbMonthToday, $dbYearToday);
  sscanf($event["start_date"], "%4s-%2s-%2s %5s", $dbYearStart, $dbMonthStart, $dbDayStart, $dbTimeStart);
  $start_date = shortDate($dbDayStart, $dbMonthStart, $dbYearStart);
  sscanf($event["end_date"], "%4s-%2s-%2s %5s", $dbYearEnd, $dbMonthEnd, $dbDayEnd, $dbEndTime);
  $end_date = shortDate($dbDayEnd, $dbMonthEnd, $dbYearEnd);
  
  $start_time = $dbTimeStart . ' Time';
  $end_time = $dbEndTime . ' Time';
  ...
  <p>
   <label title="Time (Optional, must not be empty!)&#10;Set: &#34;00:00&#34; for an all-day event.&#10;Access key: U"><u>T</u>ime:
   <input type="time" name="time_begin" value="' . $dbTimeStart . '" accesskey="u" required></label>&emsp;
